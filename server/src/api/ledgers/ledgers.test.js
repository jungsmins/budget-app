...
describe('GET /api/ledgers는', () => {
  ...
  describe('실패시', () => {
    test('limit이 숫자가 아니면 400을 반환한다.', async () => {
      ...
      expect(model.findAll).not.toHaveBeenCalledWith();
    });
  });
});
...
describe('GET /api/ledgers/:id는', () => {
  describe('성공시', () => {
    test('id에 해당하는 가계부 객체와 200을 반환한다.', async () => {
      ...
      const res = await request(app)
        .get(`/api/ledgers/${mockLedger.id}`)
        .expect(200);

      expect(res.body).toHaveProperty('id', mockLedger.id);
      expect(res.body).toHaveProperty('name', '테스트 가계부');
      expect(res.body).toHaveProperty('description', '테스트 설명');
      expect(model.findById).toHaveBeenCalledWith(mockLedger.id);
    });
  });
  ...
  describe('실패시', () => {
    ...
    test('가계부가 존재하지 않으면 404를 반환한다.', async () => {
      ...
      await request(app)
        .get('/api/ledgers/507f1f77bcf86cd799439099')
        .expect(404);
    });
  });
});
...
describe('PUT /api/ledgers/:id는', () => {
  describe('성공시', () => {
    test('id에 해당하는 가계부를 수정하고 수정된 가계부와 200을 반환한다.', async () => {
      ...
      model.update.mockResolvedValue(updateLedger);

      const res = await request(app)
        .put(`/api/ledgers/${mockLedger.id}`)
        .send({
          name: '수정된 가계부',
          description: '수정된 가계부입니다.',
        })
      ...
      expect(model.update).toHaveBeenCalledWith(mockLedger.id, {
        name: '수정된 가계부',
        description: '수정된 가계부입니다.',
      });
    });
  ...
  describe('실패시', () => {
    ...
    test('가계부가 존재하지 않으면 404를 반환한다.', async () => {
      ...
      await request(app)
        .put('/api/ledgers/507f1f77bcf86cd799439099')
        .send({
          name: '수정된 가계부',
          description: '수정된 가계부입니다.',
        })
...
describe('DELETE /api/ledgers/:id는', () => {
  describe('성공시', () => {
    test('id에 해당하는 가계부를 삭제하고 204를 반환한다.', async () => {
      ...
      await request(app).delete(`/api/ledgers/${mockLedger.id}`).expect(204);

      expect(model.remove).toHaveBeenCalledWith(mockLedger.id);
    });
  });
  ...
  describe('실패시', () => {
    ...
    test('가계부가 존재하지 않으면 404를 반환한다.', async () => {
      ...
      await request(app)
        .delete('/api/ledgers/507f1f77bcf86cd799439099')
        .expect(404);
    });
  });
});
